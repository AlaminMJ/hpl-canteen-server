...
export const createMealConsumption = async (req: Request, res: Response) => {
  const { employee_Id, meal_type, date, status = 'Pending', deviceinfo } = req.body;
...
//   }
  ...
  try {
    ...
    if (err.code === 11000) {
      return res.status(409).json({ message: 'Duplicate record exists for this employee, meal, and date' });
    }
